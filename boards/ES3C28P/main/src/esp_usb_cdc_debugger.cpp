...
bool FosDbg::sendData(uint8_t *data, uint32_t length) {
    while(length) {
    ...
    while(1) {
        if(xSemaphoreTake(cdcRxSemaphore, portMAX_DELAY) == pdTRUE) {
            ...
            if(rxSize > 0) {
                ...
                if(rxDataToltalLength && (rxDataLengthReceived >= rxDataToltalLength)) {
                    receivedDataHandler(rxData, rxDataLengthReceived);
                    rxDataToltalLength = 0;
                    rxDataLengthReceived = 0;
                }
            }
