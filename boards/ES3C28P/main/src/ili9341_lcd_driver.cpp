...
#define SPI_MAX_TRANSFER_SZ     (DISPLAY_WIDTH * DISPLAY_HEIGHT / 8 * 2)
...
static void SPI_Init(void) {
    ...
    devcfg.flags = 0;
    ESP_ERROR_CHECK(spi_bus_add_device(LCD_SPI_PORT, &devcfg, &spiHandle));
}
...
void FDev::Display::write(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t *data, uint16_t stride) {
    ...
    if(w == stride) {
        ...
        while(len > 0) {
            ...
            spi_device_transmit(spiHandle, &trans[count]);
            count = !count;
            data += sz;
        }
    }
        ...
        while(h > 0) {
            ...
            trans[count].tx_buffer = data;
            h--;
            trans[count].flags = h > 0 ? SPI_TRANS_CS_KEEP_ACTIVE : 0;
            spi_device_transmit(spiHandle, &trans[count]);
            count = !count;
            data += stride;
        }
