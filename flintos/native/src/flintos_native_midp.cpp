...
/* ---- Logical LCD dimensions (set by DisplayBridge.init) ---- */
static uint16_t lcdWidth  = DISPLAY_WIDTH;
static uint16_t lcdHeight = DISPLAY_HEIGHT;

/* ---- flintos.midp.DisplayBridge ---- */

jvoid NativeMidpLcd_Init(FNIEnv *env) {
    (void)env;
    lcdWidth  = DISPLAY_WIDTH;
    lcdHeight = DISPLAY_HEIGHT;
}
...
jvoid NativeMidpLcd_InitEx(FNIEnv *env, jint width, jint height, jstring mode) {
    ...
    (void)mode;
    if(width > 0 && height > 0) {
        lcdWidth  = (uint16_t)width;
        lcdHeight = (uint16_t)height;
    }
}
...
jvoid NativeMidpLcd_Present(FNIEnv *env, jbyteArray frameBuffer) {
    if(frameBuffer == NULL) {
        env->throwNew(env->findClass("java/lang/NullPointerException"), "frameBuffer");
        return;
    }
    uint32_t requiredLength = (uint32_t)lcdWidth * (uint32_t)lcdHeight * 2;
    if((uint32_t)frameBuffer->getLength() < requiredLength) {
        env->throwNew(env->findClass("java/lang/IllegalArgumentException"),
            "frameBuffer too small");
        return;
    }
    DisplaySrv::write(0, 0, lcdWidth, lcdHeight, (uint8_t *)frameBuffer->getData());
}
...
/* Stub: touch hardware not present on all boards.
 * Returns -1 (no touch). Override when touch driver is available. */
...
/* ESP32-S3 ROM miniz: raw deflate/zlib inflate. */
...
jobject NativeMidpPng_Decode(FNIEnv *env, jbyteArray data, jint off, jint len, jintArray wh) {
    if(data == NULL || wh == NULL) return NULL;
    if(off < 0 || len < 8 || (uint32_t)(off + len) > data->getLength()) return NULL;
    const uint8_t *png = (const uint8_t *)data->getData() + off;
    ...
    size_t expanded = (size_t)(rowBytes + 1) * H;
    ...
    for(int y = 0; y < H; y++) {
        ...
        for(int x = 0; x < W; x++) {
            ...
            if(colorType == 2) {
                ...
                r = g = b = gray;
                if(trnsCount >= 2 && gray == trns[1]) al = 0;
            } else if(colorType == 4) {
    ...
    int32_t *whp = wh->getData();
    whp[0] = W; whp[1] = H; whp[2] = hasAlpha ? 3 : 2;
    return out;
}

/* ---- flintos.midp.ResourceLoader ---- */
/* No native methods — uses Class.getResourceAsStream() from FlintJVM */
